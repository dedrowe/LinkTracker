...
public class ScrapperClient {
    ...
    public ScrapperClient(RestClient client) {
        this.client = client;
    }
    ...
    public void deleteChat(long chatId) {
        setStatusHandler(client.delete().uri("/tg-chat/{id}", chatId).retrieve()).toBodilessEntity();
    }

    public ListLinkResponse getLinks(long chatId) {
        return setStatusHandler(client.get().uri(uriBuilder -> uriBuilder.path("/links").queryParam("chatId", chatId)
            .build()).retrieve()).toEntity(ListLinkResponse.class).getBody();
    }

    public void trackLink(long chatId, AddLinkRequest request) {
        setStatusHandler(client.post().uri(uriBuilder -> uriBuilder.path("/links").queryParam("chatId", chatId)
            .build()).body(request).retrieve()).toBodilessEntity();
    }

    public void untrackLink(long chatId, RemoveLinkRequest request) {
        setStatusHandler(client.method(HttpMethod.DELETE).uri(uriBuilder -> uriBuilder.path("/links").queryParam("chatId", chatId)
            .build()).body(request).retrieve()).toBodilessEntity();
    }
}
